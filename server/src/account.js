...
const { findRoleById } = require('./role')
...
function getAccountsByInfo(list, { username, email, phone, role, createBeginTime, createEndTime, updateBeginTime, updateEndTime }) {
    //通过数组的filter方法不断筛选出符合筛选条件的结果
    let accounts = list
    if (username) {
        //若数组成员的username等于传入的username则保留
        accounts = accounts.filter(value => { return value.username === username })
    }
    if (email) {
        accounts = accounts.filter(value => { return value.email === email })
    }
    if (phone) {
        accounts = accounts.filter(value => { return value.phone === phone })
    }
    if (role) {
        accounts = accounts.filter(value => { return value.role === role })
    }
    if ((createBeginTime === 0 || createBeginTime) && (createEndTime === 0 || createEndTime)) {
        //若数组成员的createtime在createBeginTime与createEndTime之间则保留
        accounts = accounts.filter(value => { return value.createtime <= createEndTime && value.createtime >= createBeginTime })
    }
    if ((updateBeginTime === 0 || updateBeginTime) && (updateEndTime === 0 || updateEndTime)) {
        accounts = accounts.filter(value => { return value.updatetime <= updateEndTime && value.updatetime >= updateBeginTime })
    }
    return accounts
}
...
async function updateAccount(ctx, next) {
    ...
    if (id) {
        //获取body参数并检查合法性
        const { username, password, email, phone, role } = ctx.request.body
        if (username && password && (email || email === '') && (phone || phone === '') && role) {
            ...
            console.log('accounts-put:请求body参数出错,返回状态码400')
            ctx.status = 400;
            ctx.body = { msg: '请求body参数不全,请求失败' }
            return
        }
    } else {
...
async function getAccounts(ctx, next) {
    let { username, email, phone, role, pageIndex, pageSize, createBeginTime, createEndTime, updateBeginTime, updateEndTime } = ctx.query;
    if (pageIndex && pageSize) {
        ...
        let accounts = getAccountsByInfo(accountList, { username, email, phone, role, createBeginTime, createEndTime, updateBeginTime, updateEndTime })
        ...
        ctx.body = {
            ...
            data: {
                ...
                result: accounts.slice(pageIndex * pageSize, (pageIndex + 1) * pageSize), //使用slice得到数组对应页码中的成员。
                msg: '已成功完成查找'
            }
        }
        await next()
